"""Convert BlastNet lifted_hydrogen_jet data to WELL HDF5 format."""
...
def convert(traj_dir: Path, output_file: Path, dry_run: bool = False):
    ...
    # Grid files are stored flat in (Ny, Nx) order; extract 1D coords
    ...
    with h5py.File(output_file, "w") as f:
        ...
        _str_attr(f, "simulation_parameters", ["Re_jet"])
        f.attrs["Re_jet"] = float(g.get("Re_jet", 5000))
        ...
        # t0_fields — pre-allocate with chunking (1 step at a time)
        ...
        for sid in range(n_steps):
            ...
            for bname, wname in zip(scalar_blastnet, scalar_well):
                arr = load_dat(paths[bname]).reshape(Ny, Nx).T  # → (Nx, Ny)
                t0_ds[wname][0, sid, :, :] = arr

            if vel_ds is not None:
                vel_stack = np.stack(
                    [load_dat(paths[v]).reshape(Ny, Nx).T for v in vel_blastnet],
                    axis=-1)  # (Nx, Ny, n_dims)
...
def main():
    ...
    parser.add_argument("--base-path",   default="./blastnet_data")
    parser.add_argument("--output-path", default="./well_output")
    ...
    traj_dir   = base / "lifted_hydrogen_jet" / "hydrogen-jet-5000"
    output_file = outdir / "lifted_hydrogen_jet_traj_5000.hdf5"
