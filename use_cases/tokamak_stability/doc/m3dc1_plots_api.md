...
plot_flux_surface_shape(
    case_dir, time_idx, output_path,
    phi=0.0, points=250, mesh=False, bound=False, lcfs=False, dpi=150
) -> Path
```

Flux surface shape on the R,Z plane via `m3.plot_shape`.
...
plot_field_mesh(
    case_dir, time_idx, field, output_path,
    coord="scalar", phi=0.0, tor_av=1,
    units="mks", mesh=False, mesh_res=0, dpi=150
) -> Path
```
...
plot_field_vs_phi(
    case_dir, time_idx, field, output_path,
    cutr=None, cutz=None, coord="scalar",
    phi_res=100, points=250, mesh=False, bound=False,
    units="mks", dpi=150
) -> Path
```

Field amplitude vs toroidal angle φ at a fixed R or Z cut.  Exactly one of
`cutr` or `cutz` must be provided; raises `ValueError` otherwise.
...
plot_flux_average_m3(
    case_dir, time_idx, field, output_path,
    coord="scalar", fcoords="pest", units="mks", dpi=150
) -> Path
```
...
plot_line(
    case_dir, time_idx, field, output_path,
    coord="scalar", angle=0.0, zoff=0.0, phi=0.0,
    tor_av=1, units="mks", dist_from_magax=False, dpi=150
) -> Path
```

Field profile along a poloidal line via `m3.plot_line`.
...
plot_eigenfunction(
    case_dir, time_idx, field, output_path,
    coord="scalar", phit=0.0, fcoords=None,
    points=200, units="mks", dpi=150
) -> Path
```

Eigenfunction and poloidal spectrum via `m3.eigenfunction`.  Runs in `case_dir`
(required for m3dc1 to locate auxiliary files).
...
```python
plot_diagnostics(case_dir, output_path, dpi=150) -> Path
```

Diagnostic timing and iteration history via `m3.plot_diagnostics`.
