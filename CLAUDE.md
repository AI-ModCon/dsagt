...
- `contract.py` — the sample-contract schema: `validate_contract` / `load_contract` / `save_contract` for `<project>/dataset_contract.yaml`, the user-facing pivot artifact the dataset builder derives from (sample keys, collation, split policy, normalization ownership, pipeline fingerprint, producer/consumer reconciliation). See `docs/dataset-contract.md`.
