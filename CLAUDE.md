...
- `dsagt init` is the one place collections are provisioned; `dsagt-server` opens only `<project>/kb_index`. The shared `kb_index/codes` collection holds the built-in tools and the base-skill codes, embedded once and stamped (`setup_core_kb.CODES_STAMP_FILE`); init copies it and loads no embedding model.
