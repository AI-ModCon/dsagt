...
DSAgt can run [AIDRIN](https://github.com/idtlab/AIDRIN) (AI Data Readiness Inspector) as the check before and after every pipeline stage. The `aidrin` skill is installed in every project from the AIDRIN repository whether or not the assessment is on; the assessment makes the agent apply it at every tabular stage. The assessment is an opt-in at `dsagt init`; off by default.

The pipeline-builder instructions already require a paired check around every data operation, with reports in `audit/`. Without the assessment, the agent writes its own check code for each stage. With it, the check for any tabular stage is a fixed AIDRIN metric set, so every stage of a pipeline is measured the same way and the before/after delta is comparable across stages and projects.

## Enable

Answer **yes** to "Enable the AIDRIN AI-readiness assessment?" in the `dsagt init` menu. (The automation path is `dsagt init <name> --agent <agent> --readiness aidrin`, with `--readiness-executable PATH` to use an AIDRIN you already installed.)
...
Every `aidrin` command in the project, assessment run or not, goes through `dsagt-run --code aidrin` so it is recorded; the agent uses the AIDRIN CLI.
...
2. The assessment is fixed: assessment runs skip the skill's intent-elicitation and plan-confirmation steps. The skill's full workflow is for readiness assessments the user asks for beyond the profile.
...
collections and skill sources unchecked (the assessment needs neither), and answer
**yes** to "Enable the AIDRIN AI-readiness assessment?". Init installs AIDRIN on first
...
At each stage the agent should run the AIDRIN assessment on the stage input before the operation and on
...
Afterwards, `ls ~/dsagt-projects/assessment-demo/audit` shows the six reports and `trace_archive/`
holds one record per metric of each assessment run and one per operation. Clean up with `dsagt rm assessment-demo -y`.

## Demos

The [cryo-EM curation demo](use-cases/cryoem.md) runs with the assessment enabled on real scientific data — the assessment measures the particle-curation step unprompted. The [AIDRIN tour](use-cases/aidrin_full_tour.md) drives quality, fairness, and privacy metrics on a tabular dataset.
