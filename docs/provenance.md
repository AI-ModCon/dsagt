...
Claude Code moves a shell command that outruns its timeout to the background; the record is written when the command exits, so leave the session open until it does. A command that asks for no timeout gets `BASH_DEFAULT_TIMEOUT_MS`, two minutes, and `BASH_MAX_TIMEOUT_MS`, ten minutes, caps what one may ask for, so keeping a long code in the foreground means raising both for the project: `{"env": {"BASH_DEFAULT_TIMEOUT_MS": "3600000", "BASH_MAX_TIMEOUT_MS": "3600000"}}` in `.claude/settings.json`.
