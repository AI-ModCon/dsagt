...
AIDRIN as the check code around every tabular pipeline stage. Enabling the gate at `dsagt init` installs AIDRIN once and appends the gate rules to the agent's instructions; the agent runs a metric profile before and after each data transformation and reports the delta.
