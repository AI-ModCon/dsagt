...
**Run a registered code in the foreground and wait for it to exit.** The execution record is written when the process exits; a turn that ends while the code is still running loses the record, and in a headless session the process itself. A long run is still waited for: set the shell tool's timeout for the run's expected length, since the tool's default is shorter than many codes take. A shell tool has a ceiling on one command (ten minutes on Claude Code, after which the harness moves the command to the background and ends it with the turn), so a run that may exceed it goes to a subagent whose result you wait for before replying, or is split into calls each under the ceiling (one sample per call). Never launch a registered code as a background shell task or in a background subagent; both end with the turn.
