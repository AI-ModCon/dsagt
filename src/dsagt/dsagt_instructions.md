...
**Run a registered code in the foreground and wait for it to exit.** The execution record is written when the process exits; a turn that ends while the code is still running loses the record, and in a headless session the process itself. For a code you expect to run more than a few minutes, hand the run to a subagent that runs it in the foreground and reports back, so the conversation stays usable in the meantime. Never launch a registered code as a background shell task.
